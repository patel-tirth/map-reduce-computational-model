...
package Generation
import MRJobs.TypesOfLogMessages.TypesOfLogMessagesMapper
...
//  }
  ...
  it should "check the date format" in {
    val dateParsed = TypesOfLogMessagesMapper.checkDateFormat("19:56:18.200")
    dateParsed shouldBe true

  }

  it should "check the date format" in {
    val dateParsed = TypesOfLogMessagesMapper.checkDateFormat("19:56:18:201")
    dateParsed shouldBe false
  }


  it should "check the predefined start time from config file" in {
    val pattern = config.getString("randomLogGenerator.TimeFrom")
    pattern shouldBe "19:56:18.200"
  }

  it should "check the predefined end time from config file" in {
    val pattern = config.getString("randomLogGenerator.TimeTo")
    pattern shouldBe "19:56:18.720"
  }
  ...
  it should "check the time format" in {
    val dateParsed = TypesOfLogMessagesMapper.checkDateFormat("19:5:20")
    dateParsed shouldBe false
  }
